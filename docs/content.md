...
hopper:      { stations: [],       feeds: 2.5, ... }
```

`travel` is **seconds to carry an item one tile** and `feeds` is **seconds
between items drawn from behind and pushed on**, both `0` for the appliances
that move nothing. They are the whole of what makes a conveyor a conveyor and a
hopper a hopper, so a faster one of either is a number rather than a new kind —
see [automation](automation.md). A row with no station cooks nothing and burns
...
ingredient out — the crate, and only the crate. It is also what a hopper looks
for behind itself, so "sold full" and "a machine may draw from this" are one
