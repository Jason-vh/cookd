...
| --- | --- |
| `src/sim/sim.test.ts` | The game, driven the way a player drives it — `PlayerInput` into `step()`, which is only possible because the sim is pure. The full pizza pipeline, hold-to-chop semantics, burning, collision regressions, the day/build loop (a room waking into its morning, the day turning at close, and the rent — that the early days are free, that a shortfall becomes a debt rather than a refusal, that clearing it keeps the kitchen, that a second closing in the red ends the run, and that an evicted kitchen will not open another day), walls between blocks (that the divider costs no floor, that a chef is stopped by the line and goes round through the gap, and that a square against the wall has three sides), the paving (walking it, what may be built on it, that what is for sale stands on it, and that an appliance against the shell can only be worked from inside), what the kitchen says is wrong with it — including that a healthy one says **nothing**, which is the assertion that keeps the warnings worth reading — and that a burnt dish costs a plate for the day and not a minute longer, which is the fact the shop's do-not-sell list is sized against, the dining room (seating, delivery, the wrong dish, tips, bussing, walkouts, a walled-off room, the closing beat, and the line at the door — that it forms, that it ends, that it is served from the front, that a rush is several people rather than a shorter clock, and that a party sits at one table with a chair each, is fed one plate at a time to whoever ordered it, and leaves one pile of plates and both tips), the drive-through (that cars queue along the lane and shuffle forward, that the car takes the food and leaves the plate behind dirty, that a dish on the sill is handed to whoever pulls up to it, that only the front car can be served — which is the whole reason the level exists — and that a room with no dining room is not a room missing one), and the plate economy — including one test that **counts the kitchen's plates** through a serve, a binning, a disconnect, closing time and an appliance being lifted, because a plate that stops existing is a room that stops working. |
| `src/sim/shop.test.ts` | The shop, through `Grab` and nothing else — because "no new verbs" is a design claim a test can hold to account. Buying, the broke refusal changing *nothing*, same-morning refunds, sellback at half, the essential kinds it will not take, that everything for sale stands on unbuildable paving, a morning's stock being a layout change the server actually sends, and a bought plate followed through a whole day loop by the conservation count. Two things here cannot be caught any other way: **two Hosts on one seed** must stock identically for five days (otherwise the bug is "my friend sees a different shop"), and demand must measurably follow free tables in both directions, with the day curve still acting as a floor. |
| `src/sim/cards.test.ts` | The menu and the recipe card, through `Grab` and nothing else, for the same reason the shop is: **zero new verbs** is a design claim a test can hold to account. A fresh room being salad-only and everybody ordering it, a card in the delivery every morning, tier-weighted and prereq-respecting, priced by tier and refused when the till is short, buying it as a held ghost, putting it back for a full refund, and the dish unlocking only when it is set down *inside* — with deliveries landing exactly the missing kit around the tile chosen, and the pathological "nowhere to put it" refused out loud while the card stays in your hands. Three things here cannot be caught any other way: **two Hosts on one seed** must be offered the same card, the shop must not offer a fryer to a kitchen with nothing to fry, and **a card must never deliver an upgrade** — the only thing standing between a $100 card and a $320 bell oven. |
...
| `src/game/latency.test.ts` | **How long the game takes to answer you, in milliseconds.** Two real `NetGame`s and the real `Host`, with a virtual clock and a pipe of whatever round trip is asked for between them, measuring the only things a player can perceive: press a button and wait for the *drawn* world to show it, then wait for the other browser to show it. Moving and handling are both one tick at any latency; watching somebody else is a round trip plus a broadcast plus the playout buffer. Swept across the whole broadcast cycle, because a single press can land just before a frame goes out or just after one did, and 50ms of that difference says nothing about the link. It also holds the machinery to account: the input queue does not ratchet, a press brings a frame forward without costing one, mispredictions are taken back, and a predicted tick never talks. And it **prints the wire budget** — bytes per frame and per player per second, with the number of chefs and customers stated — because the two figures that lived in prose instead were each measured once under conditions nobody wrote down, and spent months disagreeing by a factor of two. |
...
cookd.world.day = 8            // then close, to see a later day's delivery
...
- The wire frame stays under 1.5 KB with four chefs — `host.test.ts`, and
  `latency.test.ts` for a kitchen that is actually being played in.
