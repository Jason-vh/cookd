...
describe("placement", () => {
  test("nothing lands on the kitchen or its patio", () => {
    ...
    // dining room.
    const placed = scatter([entry("tree", 40), entry("tuft", 120)], SPACE, 10, 5, seeded(1));
  ...
  test("props stay inside the ring their entry asks for", () => {
    const placed = scatter([entry("rock", 30, 12, 15)], SPACE, 10, 5, seeded(3));
  ...
  test("the same seed scatters the same park", () => {
    ...
    const a = scatter(entries, SPACE, 10, 5, seeded(99));
    const b = scatter(entries, SPACE, 10, 5, seeded(99));
    expect(a).toEqual(b);
  });

  test("a different seed scatters a different park", () => {
    const entries = [entry("tree", 20)];
    const a = scatter(entries, SPACE, 10, 5, seeded(1));
    const b = scatter(entries, SPACE, 10, 5, seeded(2));
    expect(a).not.toEqual(b);
  });

  test("largest first, so the hard-to-fit props get the space", () => {
    const placed = scatter([entry("tuft", 5), entry("picnic", 3)], SPACE, 10, 5, seeded(5));
...
describe("when it cannot fit everything", () => {
  test("it under-delivers rather than overlapping or hanging", () => {
    ...
    const placed = scatter(impossible, SPACE, 10, 5, seeded(11));
  ...
  test("an empty biome places nothing and does not throw", () => {
    expect(scatter([], SPACE, 10, 5, seeded(1))).toEqual([]);
    expect(scatter([entry("tree", 0)], SPACE, 10, 5, seeded(1))).toEqual([]);
  });
});
